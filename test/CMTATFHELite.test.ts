...
describe('CMTATFHELite', function () {
  ...
  describe('decimals configuration', function () {
    it('uses configured decimals for CMTATFHELite', async function () {
      const { token } = await deployToken('CMTATFHELite', 3);
      expect(await token.decimals()).to.equal(3);
    });

    it('reverts with CMTAT_DecimalsTooHigh when decimals > 18', async function () {
      const [admin] = await ethers.getSigners();
      const factory = await ethers.getContractFactory('CMTATFHELite');
      await expect(
